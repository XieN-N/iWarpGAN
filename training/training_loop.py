...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    return (gw, gh), np.stack(images1), np.stack(images2), np.stack(labels1), np.stack(labels2)
...
def training_loop(
    ...
    if rank == 0:
        ...
        grid_size, images1, images2, labels1, labels2 = setup_snapshot_image_grid(training_set=training_set)
        save_image_grid(images1, os.path.join(run_dir, 'reals-ES.png'), drange=[0,255], grid_size=grid_size)
        save_image_grid(images2, os.path.join(run_dir, 'reals-ED.png'), drange=[0,255], grid_size=grid_size)
        grid_z = torch.randn([labels1.shape[0], G.z_dim], device=device).split(batch_gpu)
        grid_c = torch.from_numpy(labels1).to(device).split(batch_gpu)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            phase_real_img1, phase_real_img2, phase_real_c1, phase_real_c2 = next(training_set_iterator)
            ...
            phase_real_c1 = phase_real_c1.to(device).split(batch_gpu)
            phase_real_c2 = phase_real_c2.to(device).split(batch_gpu)

            all_gen_z1 = torch.randn([len(phases) * batch_size, G.z_dim//2], device=device)
            all_gen_z1 = [phase_gen_z.split(batch_gpu) for phase_gen_z in all_gen_z1.split(batch_size)]

            all_gen_z2 = torch.randn([len(phases) * batch_size, G.z_dim//2], device=device)
            all_gen_z2 = [phase_gen_z.split(batch_gpu) for phase_gen_z in all_gen_z2.split(batch_size)]
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
            save_image_grid(images, os.path.join(run_dir, f'fakes-G-{cur_nimg//1000:06d}.png'), drange=[-1,1], grid_size=grid_size)
